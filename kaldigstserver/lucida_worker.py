...
coach_service = os.environ.get('LUCIDA_BANKCOACH')
if not coach_service == None:
    ...
    from bankcoachservice import BankCoachService
    from bankcoachservice.ttypes import *

    coach_toks = coach_service.split(':')
    assert len(coach_toks) == 2
    socket = TSocket.TSocket(coach_toks[0], int(coach_toks[1]))
    transport = TTransport.TBufferedTransport(socket)
    protocol = TBinaryProtocol.TBinaryProtocol(transport)
    client = BankCoachService.Client(protocol)
    transport.open()
    def call_bankcoach(transcript):
        return client.infer('bankjoy-demo', lucidatypes.ttypes.QuerySpec(name=transcript))
    ...
    def send_adaptation_state(self):
        if hasattr(self.decoder_pipeline, 'get_adaptation_state'):
            ...
            event = dict(status=common.STATUS_SUCCESS,
                         ...
                         coachresponse=call_bankcoach(self.coachtranscript),
